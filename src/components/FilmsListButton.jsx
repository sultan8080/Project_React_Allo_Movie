...
const FilmsList = () => {
  ...
  const handleQuery = (event) => {
    ...
    if (query.length > 0) {
      const searchdata = films.filter((film) =>
  ...
  return (
    <>
        ...
        <form onSubmit={handleQuery}>
          <div className="flex flex-col lg:flex-row ">
            <input
              ...
              placeholder="Entrer le nom de film"
              aria-label="search"
              onChange={handleChange}
              value={query}
            ></input>
            <button
              className="border-cyan-400 bg-cyan-700 hover:bg-cyan-400 w-full lg:w-1/3 text-white"
              type="submit"
            >
      ...
      <div className="mt-8  gap-5 flex flex-wrap justify-center">
        {films.length > 0 ? (
          films.map((films) => <FilmsListDisplay key={films.id} film={films} />)
        ) : (
